...
export function AdminArticle() {
    ...
    function load() {
        axios.get(`${API_BASE_URL}courses/articles`, {
            headers: {
    ...
    function addMaterial() {
        ...
        if (subject.length > 0 && theme.length > 0 && video.length > 0 && title.length > 0) {
            axios.post(`${API_BASE_URL}admin/article`, { part_title: theme, link: video, title }, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            })
                .then((res) => {
                    ...
                    onClose();
                    toast({
                        title: 'Успешно', description: "Вы успешно добавили статью", status: 'success', duration: 4000, isClosable: true, position: 'bottom-right'
                    });
                })
    ...
    function lectionDelete(id) {
        axios.delete(`${API_BASE_URL}admin/article/${id}`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
        })
            .then((res) => {
                ...
                load();
                toast({
                    title: 'Успешно', description: "Вы успешно удалили статью", status: 'success', duration: 4000, isClosable: true, position: 'bottom-right'
                });
            })
            .catch((e) => console.log(e));
        console.log(id);
                    ...
                    <div className={styles.modalSignIn}>
                        ...
                        <div className={styles.modal_body}>
                            <div className={styles.modal_inputBlock}>
                                ...
                                <input placeholder="Ссылка на статью" value={video} onChange={(e) => setVideo(e.target.value)} className={styles.modal_input} />
                                <input placeholder="Название статьи" value={title} onChange={(e) => setTitle(e.target.value)} className={styles.modal_input} />
                            </div>
                            <div className={styles.modal_buttonBlock}>
                                <Button isLoading={loading} backgroundColor='#07C88E' borderRadius='8px' height='56px' width='100%' color='white' _hover={{}} fontWeight={500} onClick={addMaterial} >Добавить статью</Button>
                            </div>
                        </div>
                    </div>
                </ModalBody>
