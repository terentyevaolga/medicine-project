...
export function AdminLections() {
    ...
    function load() {
        axios.get(`${API_BASE_URL}courses/video_materials`, {
            headers: {
    ...
    function addMaterial() {
        setLoading(true);
        if (subject.length > 0 && theme.length > 0 && video.length > 0 && title.length > 0) {
            axios.post(`${API_BASE_URL}admin/source`, { part_title: theme, link: video, title }, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            })
                .then((res) => {
                    ...
                    onClose();
                    toast({
                        title: 'Успешно', description: "Вы успешно добавили лекцию", status: 'success', duration: 4000, isClosable: true, position: 'bottom-right'
                    });
                })
                .catch((e) => { console.log(e); setLoading(false); });
        } else {
            console.log(course, subject, theme, video, title)
            setLoading(false);
        }
    };
    ...
    function lectionDelete(id) {
        axios.delete(`${API_BASE_URL}admin/source/${id}`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
        })
            .then((res) => {
                ...
                load();
                toast({
                    title: 'Успешно', description: "Вы успешно удалили лекцию", status: 'success', duration: 4000, isClosable: true, position: 'bottom-right'
                });
            })
            .catch((e) => console.log(e));
        console.log(id);
        ...
        <div className={styles.container}>
            ...
            <div className={styles.materialBlock}>
                ...
                <div className={styles.materialBox}>
                    {data.map((x, num) => x.themes.map(y => y.parts.map(z => z.sources.map((n, i) => String(num).includes(sortCourse) && z.title.includes(sortSubject) && n.title.toLowerCase().includes(sort.toLowerCase()) && <div key={i} className={styles.tableItem}>
                    ...
                    <div className={styles.modalSignIn}>
                        ...
                        <div className={styles.modal_body}>
                            <div className={styles.modal_inputBlock}>
                                ...
                                <input placeholder="Ссылка на видео" value={video} onChange={(e) => setVideo(e.target.value)} className={styles.modal_input} />
                                <input placeholder="Название видео" value={title} onChange={(e) => setTitle(e.target.value)} className={styles.modal_input} />
                            </div>
                            <div className={styles.modal_buttonBlock}>
                                <Button isLoading={loading} backgroundColor='#07C88E' borderRadius='8px' height='56px' width='100%' color='white' _hover={{}} fontWeight={500} onClick={addMaterial} >Добавить лекцию</Button>
                            </div>
                        </div>
                    </div>
                </ModalBody>
