...
export function AdminMetodich() {
    ...
    function addMaterial() {
        ...
        file.current.append('type', 'manual');
            ...
            try {
                axios.post(`${API_BASE_URL}admin/file-source-text`, file.current, {
                    ...
                    .then((res) => {
                        ...
                        onClose();
                        toast({
                            title: 'Успешно', description: "Вы успешно добавили методичку", status: 'success', duration: 4000, isClosable: true, position: 'bottom-right'
                        });
                    })
    ...
    function lectionDelete(id) {
        axios.delete(`${API_BASE_URL}admin/file-source/${id}`, {
            ...
            .then((res) => {
                ...
                load();
                toast({
                    title: 'Успешно', description: "Вы успешно удалили методичку", status: 'success', duration: 4000, isClosable: true, position: 'bottom-right'
                });
            })
            .catch((e) => console.log(e));
    };
        ...
        <div className={styles.container}>
            ...
            <div className={styles.materialBlock}>
                ...
                <div className={styles.materialBox}>
                    {data.map((x, num) => x.themes.map(y => y.parts.map(z => z.sources.map((n, i) => n.type === 'manual' && String(num).includes(sortCourse) && z.title.includes(sortSubject) && n.title.toLowerCase().includes(sort.toLowerCase()) && <div key={i} className={styles.tableItem}>
                    ...
                    <div className={styles.modalSignIn}>
                        ...
                        <div className={styles.modal_body}>
                            <div className={styles.modal_inputBlock}>
                                ...
                                <input placeholder="Название методички" value={title} onChange={(e) => setTitle(e.target.value)} className={styles.modal_input} />
                                {fileName !== ''
                                    ? <p>{fileName}</p>
                            ...
                            <div className={styles.modal_buttonBlock}>
                                <Button isLoading={loading} backgroundColor='#07C88E' borderRadius='8px' height='56px' width='100%' color='white' _hover={{}} fontWeight={500} onClick={addMaterial} >Добавить методичку</Button>
                            </div>
                        </div>
                    </div>
                </ModalBody>
